...
By contributing to shared repositories—whether with code (commits), issues, or comments—archaeologists using GitHub form a collaborative network which we can map using data from the GitHub API [@fig-graph-repo-user].
Here we consider two facets of this network: repositories connected by common contributors (the repository–repository graph, @fig-graph-repo-repo), and users connected by contributions to common repositories (the user–user graph, @fig-graph-user-user).
...
We identified clusters using the edge-betweenness community detection method, which operates by locating the edges that are situated along the shortest paths between all pairs of nodes, and which therefore exhibit high betweenness centrality; these edges form indirect links between otherwise completely unconnected sections of the network, and as such are the loci that distinguish groups of nodes that are more highly connected to each other than they are to others [@girvan2002].
...
```{r setup-girafe}
set_girafe_defaults(
  opts_zoom = opts_zoom(min = 1, max = 9) 
)
```
...
g_repo_user <- oarch_graph_repo_user |>
  ...
  geom_point_interactive(
    aes(x, y, colour = cluster, shape = node_type, tooltip = node),
    alpha = 0.8
  ) + 
  scale_edge_alpha(trans = "log", guide = guide_none(), range = c(0.05, 0.25)) +
  scale_colour_discrete(guide = guide_none()) +
  labs(shape = NULL) +
  theme(panel.background = element_rect(fill = "white"), 
        legend.position = "bottom")

if(knitr::is_html_output()) girafe(ggobj = g_repo_user) else g_repo_user

# export to HTML
export_as_html <- TRUE
if(export_as_html){
  g_html <- girafe(ggobj = g_repo_user)
  htmlwidgets::saveWidget(g_html,  here("analysis/data/derived_data/oarch_repo_user.html"), selfcontained = TRUE)
}
```

```{r fig-graph-repo-repo}
#| fig-cap: Graph of open archaeology repositories connected by common contributors. Darker edges indicate a great number of common contributors. Node colour indicates membership of the largest clusters according to the edge–betweenness method [@girvan2002]. Excludes isolate nodes.
g_repo_repo <- oarch_graph_repo_repo |>
  ...
  geom_point_interactive(
    aes(x, y, colour = cluster, tooltip = repo),
    alpha = 0.8
  ) + 
  scale_edge_alpha(guide = guide_none(), range = c(0.05, 0.25)) +
  scale_colour_discrete(guide = guide_none()) +
  theme(panel.background = element_rect(fill = "white"))

if(knitr::is_html_output()) girafe(ggobj = g_repo_repo) else g_repo_user
```

```{r fig-graph-user-user}
#| fig-cap: Graph of open archaeology users connected by contributions to common repositories. Darker edges indicate a great number of common repositories. Node colour indicates membership of the largest clusters according to the edge–betweenness method [@girvan2002]. Excludes isolate nodes.
g_user_user <- oarch_graph_user_user |>
  ...
  geom_edge_link(aes(alpha = n)) +
  geom_point_interactive(
    aes(x, y, colour = cluster, tooltip = user),
    alpha = 0.8
  ) + 
  scale_edge_alpha(guide = guide_none(), range = c(0.05, 0.25)) +
  scale_colour_discrete(guide = guide_none()) +
...
if(knitr::is_html_output()) girafe(ggobj = g_user_user) else g_user_user
```
...
This indicates that most repositories that have had more than one contributor, or whose contributors have worked independently on more than one repository, are not isolated, and that at least one of their contributors have, at least one time, worked with members of a broader interconnected population.

We delimited `r nrow(filter(clusters_n_repos, n > 1))` distinct clusters that outline the topography of the repository–repository network (@fig-graph-repo-repo).
...
Clustering also reveals distinct collaborative networks within the user-user graph (@fig-graph-user-user).
We again see a complementary primary interconnected subgraph and several unconnected subgraphs.
The primary subgraph comprises several connected clusters, including one central cluster and several more peripheral clusters, which are internally-cohesive and exhibit few connections with other peripheral clusters.
The central cluster bridges all the peripheral clusters.
Moreover, the central cluster is not uniform, and comprises several relatively discrete components representing collaborative sub-communities.
While these components are internally cohesive, they exhibit enough connections to other members of the central cluster so as to not be considered as separate or peripheral clusters.
...
This is evident through the fact that many of the connections represented in the cores emerge from more conventional professional networks, e.g. the [ISAAKiel](https://isaakiel.github.io/) group based at the University of Kiel or [CAA-SSLA](https://sslarch.github.io/), a special interest group of the international scholarly society 'Computational and Quantitative Applications in Archaeology' (CAA) focused on scientific programming, and which clusters around users 'nevrome' (Clemens Schmid) and 'martinHinz' (Martin Hinz), who are core members of both organizations.
...
oarch_graph_repo_repo |>
  ...
  mutate(repo = glue("[{repo}](https://github.com/{repo})")) |>
...
#| fig-cap: Repository centrality by age (left) and length (right). Centrality is measured by node betweenness weighted by number of contributions. Blue line indicates a generalized linear regression, with confidence envelope shaded in grey.
...
On the other hand, standalone software tend to integrate multiple system components and may evolve over time to add new features or support new workflows.
...
Python projects, which tend to relate to the development of information infrastructures or are of interest to members of other fields (e.g., palaeo-ecology, other fieldwork-based disciplines), are also highly ranked in terms of average centrality.
Further analysis is warranted to qualify these observations regarding the significance of development patterns when working across different languages and platforms.

```{r fig-user-betweenness-contribs}
#| fig-cap: User centrality by total number of contributions. Centrality is measured by node betweenness weighted by number of contributions.  Blue line indicates a generalized linear regression, with confidence envelope shaded in grey.
oarch_graph_user_user |>
  as_tibble() |> 
